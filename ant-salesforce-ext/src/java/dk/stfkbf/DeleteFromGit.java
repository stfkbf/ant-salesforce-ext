	...
	public void traverse(Git git, File file) throws Exception{		
		...
		if (file.exists()){
			for (String currentFile : targetDir.list()){
				...
				File sourceFile = new File(targetFile.getAbsolutePath().replace(this.getTargetPath(), this.getSourcePath()));

				String fileName = targetFile.getAbsolutePath().replace(this.gitRoot + "/", "");
